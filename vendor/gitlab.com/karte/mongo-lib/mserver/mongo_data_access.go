...
func (dal *mongoDataAccessLayer) Get(id string, resource interface{}) (result interface{}, err error) {
	...
	collection := worker.DB().C(pluralizeLowerResourceName(resourceType))
...
func (dal *mongoDataAccessLayer) GetAll(resource interface{}) (results []interface{}, err error) {
	if err != nil {
		return nil, convertMongoErr(err)
	}
...
func (dal *mongoDataAccessLayer) PostWithID(id string, resource interface{}) error {
	...
	idField := reflect.ValueOf(resource).Elem().FieldByName("Id")
	idField.SetString(bsonID.Hex())
...
func (dal *mongoDataAccessLayer) ConditionalDelete(query search.Query) (count int, err error) {
	...
	if dal.hasInterceptorsForOpAndType("Delete", resourceType) {
		...
		*/
		...
		bundle, err = dal.Search(url.URL{}, query, nil) // the baseURL argument here does not matter
		...
		if err == nil {
			...
			if count < len(IDsToDelete) {
				...
				failBundle, searchErr = dal.Search(url.URL{}, query, nil)
...
func (dal *mongoDataAccessLayer) Search(baseURL url.URL, searchQuery search.Query, resourceType interface{}) (*models.Bundle, error) {
	...
	result, total, err := searcher.Search(searchQuery, resourceType)
	if err != nil {
		return nil, convertMongoErr(err)
	}
...
func (dal *mongoDataAccessLayer) FindIDs(searchQuery search.Query) (IDs []string, err error) {
	...
	searcher := search.NewMongoSearcher(worker.DB(), dal.countTotalResults, dal.enableCISearches, dal.readonly)
	results, _, err := searcher.Search(newQuery, nil)
	if err != nil {
		return nil, convertMongoErr(err)
	}
...
func updateLastUpdatedDate(resource interface{}) {
	...
	now := &models.KarteDateTime{Time: time.Now(), Precision: models.Timestamp}
...
func pluralizeLowerResourceName(resourceType string) string {
