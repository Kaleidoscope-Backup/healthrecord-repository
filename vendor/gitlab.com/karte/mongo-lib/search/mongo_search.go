...
// MongoSearcher implements KARTE searches using the Mongo database.
...
func sliceForResource(resource interface{}, len int, cap int) interface{} {
	rType := reflect.TypeOf(resource)
	return reflect.MakeSlice(reflect.SliceOf(rType), len, cap).Interface()
}

func newSliceForResource(resource interface{}, len int, cap int) interface{} {
	rSlice := sliceForResource(resource, len, cap)
...
func (m *MongoSearcher) Search(query Query, resourceType interface{}) (results interface{}, total uint32, err error) {
	...
	if query.UsesIncludes() || query.UsesRevIncludes() {
		...
		results = newSliceForResource(resourceType, 0, 0)
	}
...
func (m *MongoSearcher) aggregate(bsonQuery *BSONQuery, options *QueryOptions, doCount bool) (pipe *mgo.Pipe, total uint32, err error) {
	c := m.db.C(pluralizeLowerResourceName(bsonQuery.Resource))
...
// database -- but the KARTE spec calls for this sort of behavior to correctly implement these
