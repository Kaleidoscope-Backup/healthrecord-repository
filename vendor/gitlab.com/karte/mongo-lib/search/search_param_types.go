...
const (
	...
	OffsetParam        = "_offset" // Custom param, not in KARTE spec
	FormatParam        = "_format"
)
...
// Query describes a string-based KARTE query and the resource it is associated
...
func (q *Query) Params() []SearchParam {
	...
	for _, queryParam := range queryParams.All() {
		...
		if param == "_has" {
			...
			info, ok = SearchParameterDictionaryKarte[q.Resource][param]
		}
...
// http://acme.com/Condition?patient=123&onset=2012&_sort=_lastUpdated:desc
// where option is determined by _{queryType}:{modifier}.{postfix}={field}
	...
	for _, queryParam := range queryParams.All() {
		...
		case SortParam:
			...
			for _, key := range keys {
				...
				sortParam, ok := SearchParameterDictionaryKarte[q.Resource][strings.TrimPrefix(key, "-")]
				if !ok {
...
// SearchParamInfo contains information about a KARTE search parameter,
...
// be searched, as well as the KARTE type of that property (e.g., "dateTime").
...
// following description is from the KARTE DSTU2 specification:
//
...
// description is from the KARTE DSTU2 specification:
//
...
// Date represents a date in a search query.  KARTE search params may define
...
// ParseDate parses a KARTE date string (roughly ISO 8601) into a Date object,
...
func ParseDate(dateStr string) *Date {
	...
	if m := dtRegex.FindStringSubmatch(dateStr); m != nil {
		...
		case mi != "":
			dt.Precision = Minute
		// NOTE: Skip hour precision since KARTE specification disallows it
		case d != "":
			dt.Precision = Day
		case mo != "":
			dt.Precision = Month
...
// Number represents a number in a search query.  KARTE search params may define
...
// The KARTE spec defines equality for 100 to be the range [99.5, 100.5) so we
...
// Prefix is an enum representing KARTE parameter prefixes.  The following
// description is from the KARTE DSTU2 specification:
//
...
// escapeFriendlySplit splits a KARTE parameter value, properly handling special
// KARTE escape characters and sequences.  The following description is from the
// KARTE DSTU2 specification:
//
